...
├── index.html (Homepage)
├── portfolio.html (Hub)
├── st1.html, st2.html, st3.html (Training guides)
├── wpba-checklist.html (Downloadable)
├── reflection-guide.html
├── consultation-models.html
├── hidden-curriculum.html (NEW)
├── sca.html (Overview)
├── sca-practice.html (Practice tools)
├── akt.html (Overview + IMG tips)
├── akt-questions.html (Question interface)
├── clinical-topics.html
├── data/
│   └── akt-questions.json (Ready for 200+ questions)
├── scripts/
│   ├── main.js
│   ├── practice-tools.js
│   └── case-assessment.js (Scoring system)
└── styles/
    ├── main.css (Core styles)
    ├── content.css (Content pages)
    ├── sca.css (SCA specific)
    ├── practice.css (Practice tools)
    └── clinical.css (Clinical topics)
```
