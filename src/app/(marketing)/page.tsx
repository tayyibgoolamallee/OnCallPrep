...
export default function HomePage() {
  return (
    <>
      {/* Hero Section */}
      <section className="relative overflow-hidden bg-gradient-to-b from-teal-50 via-white to-slate-50 py-24 sm:py-32 lg:py-40">
        <div className="mx-auto max-w-7xl px-6 lg:px-8 relative">
          <div className="mx-auto max-w-4xl text-center">
            <Badge className="mb-8 inline-flex px-4 py-2 text-sm font-semibold bg-teal-100 text-teal-800 border-teal-300">
              Built by trainees, for trainees
            </Badge>
            <h1 className="text-5xl font-bold tracking-tight text-slate-900 sm:text-6xl lg:text-7xl">
              Simplify GP Training.
              <br />
              <span className="text-teal-700">Say Goodbye to Overwhelm.</span>
            </h1>
            <p className="mt-8 text-lg leading-8 text-slate-700 sm:text-xl lg:text-2xl">
            ...
            <div className="mt-12 flex items-center justify-center gap-x-6">
              <Link href="/signup">
                <Button size="lg" className="h-12 px-8 text-base font-semibold bg-teal-600 text-white hover:bg-teal-700">
                  Get Started Free
                </Button>
              </Link>
              <Link href="/explore">
                <Button size="lg" variant="outline" className="h-12 px-8 text-base font-semibold border-2 border-teal-600 text-teal-700 hover:bg-teal-50">
                  Explore Resources
                </Button>
              </Link>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mx-auto mt-16 grid max-w-7xl grid-cols-1 gap-8 lg:grid-cols-3">
            {features.map((feature) => (
              <Card key={feature.title} className="relative overflow-hidden border-l-4 border-teal-600 bg-white transition-shadow hover:shadow-lg hover:border-teal-700">
                <CardHeader>
                ...
                <CardContent>
                  <ul className="mb-8 space-y-3">
                    {feature.items.map((item) => (
                      <li key={item.name} className="flex items-center gap-3">
                        <span className={`text-lg ${item.free ? 'text-teal-600' : 'text-slate-700'}`}>
                          {item.free ? '✓' : '★'}
                        </span>
                        <span className="flex-1 text-sm text-slate-700">{item.name}</span>
                        <Badge variant={item.free ? 'secondary' : 'default'} className={`text-xs ${item.free ? 'bg-teal-100 text-teal-800' : 'bg-slate-800 text-white'}`}>
                          {item.free ? 'Free' : 'Pro'}
                        </Badge>
                      </li>
                    ))}
                  ...
                  <Link href={feature.href}>
                    <Button variant="outline" className="w-full border-2 border-teal-600 text-teal-700 hover:bg-teal-50">
                      Learn More
                    </Button>
                  </Link>
                </CardContent>
      ...
      {/* How It Works */}
      <section className="bg-gradient-to-br from-teal-100 via-white to-slate-100 py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl lg:text-5xl">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mx-auto mt-16 grid max-w-7xl grid-cols-1 gap-8 lg:grid-cols-3">
            {testimonials.map((testimonial) => (
              <Card key={testimonial.author} className="border-2 border-teal-200 bg-white transition-shadow hover:shadow-lg hover:border-teal-300">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <div className="mt-10 flex items-center justify-center gap-x-6">
              <Link href="/signup">
                <Button size="lg" className="h-12 bg-teal-600 px-8 text-base font-semibold text-white hover:bg-teal-700 border-2 border-teal-600">
                  Get Started Free
                </Button>
              </Link>
              <Link href="/pricing">
                <Button size="lg" variant="outline" className="h-12 border-2 border-white px-8 text-base font-semibold text-white hover:bg-white hover:text-black bg-transparent">
                  View Pricing
                </Button>
              </Link>
            </div>
